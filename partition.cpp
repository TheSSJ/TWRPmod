...
#ifdef TW_NTFS_WA	
	//Try to mount an "auto" drive with ntfs-3g before giving up :)
	//check first if ntfs-3g is possible
	if(Actual_Block_Device != "" && ((Current_File_System == "auto"  && Removable == true) || Current_File_System == "ntfs" || Current_File_System == "ntfs-3g"))
	{
		string retString = "";
		LOGINFO("<=======Trying ntfs-3g %s %s on %s ========>\n", Actual_Block_Device.c_str(), Current_File_System.c_str(), Mount_Point.c_str());
		string ntfsCommand = "ntfs-3g " + Actual_Block_Device + " " + Mount_Point + "";
		if (TWFunc::Exec_Cmd(ntfsCommand) == 0) {
			Update_Size(false);
